...
/******************************************BEGIN HTML REPORT************************************************/
libname SQ "/home/u63515397/ESQ1M6/data";
...
ods html5 file="FinalReport.html" path="/home/u63515397";
...
/*Creating a new table with the count of overall claims by year using the work.claims_cleaned table*/
proc sql;
...
/*Visualizing the newly created table in a bar chart*/
...
/*Storing the total enplanements value in a macro variable using the work.ClaimsByAirport table*/
proc sql noprint;
...
/*Storing the total claims filed value in a macro variable*/
proc sql noprint;
...
/*Storing the percentage of claims filed value in a macro variable*/
     /*Removing commas from the macro variables*/
%let TotalEnplanementsNum=%sysfunc(compress("&TotalEnplanements",","));
%let TotalClaimsNum=%sysfunc(compress("&TotalClaims",","));
     /*Calculating the percentage of total claims by enplanements and store in a macro variable*/
data _null_;
	Total=&TotalClaimsNum/&TotalEnplanementsNum;
	PercentValue=put(Total,percent8.4);
	call symputx('PctClaimsTotal',PercentValue);
run;
%put Percentage of Claims by Enplanements (Total): &PctClaimsTotal;
...
/*Storing the average days of claim value in a macro variable*/
proc sql noprint;
...
/*Storing average days of claim value in a macro variable*/
proc sql noprint;
...
/*Visualizing the frequency of Claim_Type*/
...
/*Visualizing the frequency of Disposition*/
ods graphics on /width=5.5in height=4in ;
...
/*Visualizing the frequency of Claim_Site*/
ods graphics on /width=5.5in height=4in;
...
/*Creating a tablet view the top 20 airports by PctClaims with over 10,000,000 passengers*/
proc sql outobs=20;
...
/*Visualizing the top20airports table*/
...
/*Storing values of the airport with the highest PctClaims for airports over 10,000,000 passengers.*/
proc sql noprint;
